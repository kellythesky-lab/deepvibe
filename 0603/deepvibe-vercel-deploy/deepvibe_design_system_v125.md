...
| 9.22 | **신규 파일 타입 추가 시 체크리스트** | .tsx 등 추가 시 tsconfig.json + 의존성 명시 (Vercel 자동 인식에 의존 금지) |
| 9.23 | **배포 실패 시 진단 순서** | 로그 먼저 → 추측 금지 → 한 번에 한 가지만 수정 |
...
### 9.22 TypeScript / 신규 파일 타입 추가 시 체크리스트 ⭐ (v125 후반 추가)

**계기**: `api/og.tsx` 도입 시 `tsconfig.json` 누락 → Vercel 빌드 실패.
에러: `error TS17004: Cannot use JSX unless the '--jsx' flag is provided.`

```
신규 *파일 타입* (.tsx, .ts, .vue, .svelte 등) 추가 시:
...
3. 런타임 의존성 확인
   - jsx: "react-jsx" + jsxImportSource: "react"
     → react 패키지가 dependencies에 필요
   - @types/react는 devDependencies로 충분

4. 로컬 빌드 테스트
   - vercel dev 또는 vercel build로 사전 검증
   - "잘 되겠지" 가정 금지

5. 배포 후 즉시 확인
   - Vercel Deployments → Build Logs
   - Status: Building → Ready 전환 확인
   - Failed 시 즉시 Build Logs의 마지막 부분 확인
```

**왜 중요한가**:
- Vercel은 자동 인식이 강력하지만, 명시적 설정 없이는 "운"에 의존
- 한 번 통한 빌드가 다른 push에서 실패할 수 있음 (Vercel 빌드 시스템 변경, 의존성 버전 변경 등)
- 명시적 tsconfig.json + package.json 의존성 = **재현 가능한** 빌드

---

### 9.23 배포 실패 시 진단 순서 ⭐ (v125 후반 추가)

```
배포가 *실패하면*:

1. Vercel 대시보드 → Deployments → 실패한 항목 클릭
2. Build Logs의 *맨 마지막 부분* 확인 (실제 에러)
3. 에러 종류 판단:

   [TS error] → 9.22 체크리스트 적용
   [Module not found] → package.json dependency 확인
   [Edge function module unsupported] → @vercel/og 같은 패키지가 Edge 호환인지 확인
   [Out of memory] → Edge function 메모리 한계
   [Build timeout] → 의존성 너무 큼

4. 코드 자체 점검은 *나중*
   먼저 *에러 메시지*가 시작점. 추측 금지.

5. 한 번에 *한 가지만* 수정 후 배포
   여러 변경 동시 push → 어떤 게 효과 있었는지 모름
```

**디자이너 노트**:
이번 세션 (v125)에서 첫 빌드 실패 발견. tsconfig.json 누락이 명백한 원인.
교훈: **에러 메시지는 거의 항상 옳다**. 추측보다 로그.
